...
def calculate_template_ratio(template_path: str, debug_dir: str = None) -> dict:
    ...
    # o fundo dos templates é branco, invertemos pra capturar o objeto
...
def find_true_corners(contour: np.ndarray, gray_img: np.ndarray = None) -> np.ndarray | None:
    """encontra os 4 cantos de um retângulo
    abordagem:
    1. obtém o minAreaRect do contorno (dá o ângulo e centro aproximados)
    2. usa fitLine nos segmentos retos do contorno (excluindo cantos)
    3. calcula intersecções
    
    a ideia pra separar os lados dos cantos: os pontos do contorno que estão
    ...
    # obtém o retângulo orientado que melhor ajusta o contorno
    ...
    # pra cada lado do retângulo, seleciona os pontos do contorno que
    # estão próximos da reta desse lado e longe dos cantos.
    # define cada lado pela reta entre dois cantos adjacentes do box
    ...
    for p_start, p_end, side_name in sides:
        ...
        # pra cada ponto do contorno:
        # - projeta no lado pra ver a posição (0 a side_len)
        # - calcula distância perpendicular ao lado
        projections = []
        for pt in cnt_pts:
            v = pt - p_start
    ...
    # ordem dos lados: 0=top, 1=right, 2=bottom, 3=left
...
def order_points(pts: np.ndarray) -> np.ndarray:
    """ ordena os pontos de forma crescente em sentido horário: top-left, top-right, bottom-right, bottom-left.
    """
...
def line_intersection_homogeneous(p1, p2, p3, p4):
    """ intersecção por coordenadas homogêneas
    """
...
def compute_aspect_ratio(corners: np.ndarray, img_w: int, img_h: int) -> dict:
    """ calcula o aspect ratio com diferentes métodos
    """
    tl, tr, br, bl = corners
...
def _normalize_shadows(gray: np.ndarray) -> np.ndarray:
    """ remove sombras e gradientes de iluminação através de normalização por divisão
    """
    h, w = gray.shape[:2]
    blur_size = max(w, h) // 3
    if blur_size % 2 == 0:
...
def _edge_contrast_score(gray: np.ndarray, contour: np.ndarray, sample_width: int = 5) -> float:
    """ avalia a nitidez da borda de um contorno.
    """
    h, w = gray.shape[:2]
